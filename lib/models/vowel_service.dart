...
class VowelService {
  ...
  static Future<List<Vowel>> getVowels() async {
    final response = await http.get(Uri.parse('$_baseUrl/vowels'));

    if (response.statusCode == 200) {
      final List<dynamic> data = jsonDecode(response.body)['data'];
      // map each json object to a Character object
      return data.map((json) => Vowel.fromJson(json)).toList();
    } else {
      throw Exception('Failed to load vowels');
    }
  }
}
